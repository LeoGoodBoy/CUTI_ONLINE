...
public class FunctionDAO {
    ...
    private Object getAction(Session session, int crud,
        ...
        switch (crud) {
            ...
            case 4:
                return session.createQuery("FROM " + type.getSimpleName() + " ORDER BY 1 DESC").list().get(0);
            ...
            case 8:
                return session.createQuery("FROM " + type
                        .getSimpleName() + " WHERE idKaryawan = '"+key+ "' ORDER BY 1").list();
                
            case 9:
                Karyawan karyawan = (Karyawan) key;
                return session.createCriteria(type)
                        .add(Restrictions.and(Restrictions.eq("emailKaryawan", karyawan.getEmailKaryawan()),
                                Restrictions.eq("kataSandi", karyawan.getKataSandi()))).uniqueResult();
